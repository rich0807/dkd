    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="nodeRef" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="所属区域" prop="regionId">
          <!-- <el-input v-model="form.regionId" placeholder="请输入关联区域ID" />
            -->
        ...
        <el-form-item label="商圈类型" prop="businessDistrictType">
          <el-select v-model="form.businessDistrictType" placeholder="请选择商圈类型">
            <el-option
        ...
        <el-form-item label="归属合作商" prop="partnerId">
          <!-- <el-input v-model="form.partnerId" placeholder="请输入关联合作商ID" /> -->
...
const data = reactive({
  ...
  rules: {
    ...
    regionId: [
      { required: true, message: "关联区域ID不能为空", trigger: "blur" }
    ],
    partnerId: [
      { required: true, message: "关联合作商ID不能为空", trigger: "blur" }
    ],
  }
});
...
/**查询区域列表 */
function getAllArea() {
  console.log('开始查询区域列表', loadAllParams) // 添加日志
...
function getAllPartner() {
  console.log('开始查询合作商列表', loadAllParams) // 添加日志
