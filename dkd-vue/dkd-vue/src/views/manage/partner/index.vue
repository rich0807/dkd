<template>
  <div class="app-container">
     ...
     <el-table-column label="点位数" align="center" prop="nodeCount" />
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="300">
        <template #default="scope">
          ...
          <el-button link type="primary"  @click="getPanelInfo(scope.row)" v-hasPermi="['manage:partner:query']">查看详情</el-button>
    ...
    <el-row>
      ...
      <el-col :span="12" >分成比例:{{ form.shareRatio }}%</el-col> 
    </el-row>
    </el-dialog>
...
import { listPartner, getPartner, delPartner, addPartner, updatePartner,resetPassword } from "@/api/manage/partner";
...
function getPanelInfo(row) {
  console.log('getPanelInfo 被触发', row); // 检查方法是否被调用及传入的参数
  reset();
  const _id = row.id;
  getPartner(_id).then(response => {
    console.log('获取到的数据:', response.data); // 检查返回的数据
    form.value = response.data;
    partnerInfoOpen.value = true;
  }).catch(error => {
    console.error('获取合作商详情失败:', error);
    proxy.$modal.msgError('获取合作商详情失败');
  });
}
...
function resetPwd(row) {
  const _id= row.id 
  proxy.$modal.confirm('你确认重置合作商管理编号为"' + _id + '"的密码？').then(function() {
